...
@router.post(
    "/invitation/accept", response_model=AcceptInvitationResponse, status_code=200
)
def accept_invitation(
    data: AcceptInvitationRequest, session: Session = Depends(get_db_session)
):
    try:
...
def create_user(
    ...
    auth: AuthContext = Depends(requires_permission("user:create")),
):
    current_user = auth.user
    session = auth.session

    try:
        user, _invitation, token = user_service.create_user(
            session=session,
