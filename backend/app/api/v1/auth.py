...
def register_tenant(
    ...
    try:
        _registration, _token = service.submit_registration(session, data)
...
def approve_registration(token: str, session: Session = Depends(get_db_session)):
    try:
        _tenant, _user = service.approval_registration(session, token)
    except service.InvalidCredentialsError:
        raise HTTPException(status_code=400, detail="Invalid or expired approval link")

    return RegistrationApprovalResponse(message="Registration approved successfully")
