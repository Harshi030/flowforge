...
def submit_expense(
    session: Session, expense_id: uuid.UUID, tenant_id: uuid.UUID, user_id: uuid.UUID
) -> Expense:
    try:
        expense_repository = ExpenseRepository(session)
        ...
        details = {"previous_status": previous_status, "new_status": expense.status}
        ...
        notification_service.notify_expense_submitted(session=session, expense=expense)
...
def reject_expense(
    ...
    try:
        ...
        notification_service.notify_expense_rejected(session=session, expense=expense)
