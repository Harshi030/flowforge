...
from app.modules.users.repository import UserInvitationRepository, UserRepository
...
def create_invitation(
    session: Session, user_id: uuid.UUID, created_by: uuid.UUID
...
def accept_invitation(session: Session, token: str, password: str) -> None:
    try:
        token_hash = security.hash_token(token)
...
def create_user(
    ...
    try:
        ...
        role = rbac_repository.get_role_by_tenant_id_and_role_id(
            tenant_id, data.role_id
        )
...
def update_user(
    ...
    try:
        ...
        if data.role_ids is not None:
            ...
            old_roles = rbac_repository.get_roles_by_user_id(user_id=user_id)

            roles = rbac_repository.get_roles_by_ids_and_tenant(
                role_ids=data.role_ids, tenant_id=tenant_id
            )
            ...
            details["old_roles"] = [role.name for role in old_roles]

            details["new_roles"] = [role.name for role in roles]
            rbac_repository.remove_roles_from_user(user_id=user_id)

            for role in roles:
                rbac_repository.assign_role_to_user(user_id=user_id, role_id=role.id)
