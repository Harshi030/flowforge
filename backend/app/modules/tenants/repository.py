...
class TenantRegistrationRepository:
    ...
    def get_pending_by_email(self,email: str):
        return self.session.execute(
            select(TenantRegistrationRequest)
            .where(TenantRegistrationRequest.email == email)
        ).scalar_one_or_none()
        
    def get_pending_by_slug(self, slug: str):
        return self.session.execute(
            select(TenantRegistrationRequest)
            .where(TenantRegistrationRequest.slug == slug)
        ).scalar_one_or_none()
    ...
    def get_by_approval_token_hash(
        ...
        return self.session.execute(
            select(TenantRegistrationRequest)
            .where(TenantRegistrationRequest.approval_token_hash == token_hash)
        ).scalar_one_or_none()
