...
def create_payment(
    session: Session, tenant_id: uuid.UUID, expense_id: uuid.UUID, created_by: uuid.UUID
) -> Payment:
    try:
...
def get_payment(
    session: Session, tenant_id: uuid.UUID, payment_id: uuid.UUID
) -> Payment:
    payment_repository = PaymentRepository(session)
...
def pay(
    session: Session, tenant_id: uuid.UUID, payment_id: uuid.UUID, updated_by: uuid.UUID
) -> Payment:
    try:
        payment_repository = PaymentRepository(session)
        ...
        details = {"previous_status": previous_status, "new_status": payment.status}
        ...
        expense = expense_repository.get_by_id_and_tenant(
            expense_id=payment.expense_id, tenant_id=tenant_id
        )
        notification_service.notify_payment_paid(session=session, expense=expense)
