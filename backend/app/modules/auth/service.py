...
def submit_registration(session: Session, data: RegisterTenantRequest):
    try:
        ...
        pending_registartion_by_email = (
            register_request_repository.get_pending_by_email(data.email)
        )

        if pending_registartion_by_email:
            raise RegistrationAlreadyPendingError()

        pending_registration_by_slug = register_request_repository.get_pending_by_slug(
            slug
        )
        ...
        expires_at = datetime.now(UTC) + timedelta(
            days=settings.registration_approval_expire_days
        )
...
def approval_registration(session: Session, token: str):
    try:
        token_hash = security.hash_token(token)
...
def login(session: Session, data: LoginRequest, key: str) -> tuple[str, str]:
    ...
    expiry_at = datetime.fromtimestamp(expiry_at, UTC)
...
def refresh_access_token(session: Session, refresh_token: str | None) -> str:
    ...
    return security.create_access_token(user.id, user.tenant_id)
